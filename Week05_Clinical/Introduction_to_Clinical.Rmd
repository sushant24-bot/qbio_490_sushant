...
```{r}
write.csv(clinic, "/home1/mayreddy/mayreddy/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```
...
# 1. How many rows are there? How many columns? 1197 rows, 112 columns
# 2. What is the data type of $gender? String
...
```{r}
under30_over60_mask <- ifelse(clinic$age_at_diagnosis < 35 | clinic$age_at_diagnosis > 60, TRUE, FALSE)
```
...
```{r}
cleaned_clinic$survival_time <- ifelse(clinic$death_days_to == "Not Applicable", cleaned_clinic$last_contact_days_to, cleaned_clinic$death_days_to)
...
0.77, it is not statistically significant, it tells us that age and breast cancer survival are not dependent on each other. 
